...
- [📖简介](#简介)
- [🚀快速集成](#快速集成)
    - [1.Gradle配置(App模块)](#1Gradle配置app模块)
    - [2.全局初始化(必须)](#2全局初始化必须)
- [功能模块使用指南](#功能模块使用指南)
...
`lib_base`是一个 Android 基础架构库。它高度封装了 **MVP 架构**、**网络请求**、**常用 UI 组件** 及 *
...
//    implementation project(':lib_base') //(module方式)
    implementation 'com.github.yzpLan:LanBase:1.0.6' //依赖方式
}
```
...
`lib_base` 自动处理了 **ViewBinding** 的初始化以及 **Presenter** 的生命周期绑定与解绑。
...
`ApiBaseResponse` 用于接收服务器返回的标准 JSON 结构。你需要根据后端接口文档定义的字段名称（如 `code`,
`msg`, `data`）进行对应。
